...
function sagaMiddlewareFactory() {
    ...
    function sagaMiddleware(_ref2) {
        ...
        sagaMiddleware.run = (prefix, saga, ...args) => runSaga({
            context: context,
            subscribe: sagaEmitter.subscribe(prefix),
            dispatch: prefix ? (action => dispatch({...action, type: `${prefix}.${action.type}`})) : dispatch,
            getState: compose(prefix ? path(prefix.split('.')) : identity, getState),
